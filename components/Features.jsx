const metrics = [
  ...
  export default function Example() {
    return (
      <div id="expertise" className="relative bg-clear max-w-5xl mx-auto">
        <div className="absolute bottom-0 h-80 w-full xl:inset-0 xl:h-full">
          <div className="h-full w-full xl:grid xl:grid-cols-2">
            <div className="h-full sm:ml-16  xl:relative xl:col-start-2  ">
              <img
                className="h-full rounded-full w-full object-cover opacity-100 xl:absolute xl:inset-0"
              ...
              <div
                aria-hidden="true"
                className="absolute inset-x-0 top-0 h-32 bg-gradient-to-b from-green-500/10 rounded-full xl:inset-y-0 xl:left-0 xl:h-full xl:w-full "
              />
            </div>
          </div>
        </div>
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:max-w-7xl lg:px-8 xl:grid xl:grid-flow-col-dense xl:grid-cols-2 xl:gap-x-8">
          <div className="relative pt-12 pb-64 sm:pt-24 sm:pb-64 xl:col-start-1 xl:pb-24">
            ...
            <div className="mt-12 grid grid-cols-1 gap-y-12 gap-x-6 sm:grid-cols-2">
              {metrics.map((item) => (
                <p key={item.id}>
                  <span className="block text-xl font-regular font-serif italic text-black">{item.stat}</span>
